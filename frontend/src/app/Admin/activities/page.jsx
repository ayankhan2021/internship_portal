...
function ActivitiesPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const progressResponse = await axios.get('http://localhost:5000/api/admin/progress-updates', {
          headers: { Authorization: `Bearer ${token}` }
        });
  ...
  const handleSubmitFeedback = async () => {
    ...
    try {
      ...
      await axios.post(
        `http://localhost:5000/api/admin/progress-updates/${selectedProgressUpdate._id}/feedback`,
