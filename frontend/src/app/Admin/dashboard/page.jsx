...
function Dashboard() {
  ...
  const handleUpdateIntern = async (e) => {
    ...
    try {
      ...
      await axios.put(`http://localhost:5000/api/interns/${selectedIntern._id}`, formData, {
        headers: {
          Authorization: `Bearer ${token}`,
          'Content-Type': 'multipart/form-data'
        }
      ...
      if (newIntern.username || newIntern.password) {
        ...
        await axios.put(
          `http://localhost:5000/api/interns/${selectedIntern._id}/credentials`,
          credentialsData,
          { headers: { Authorization: `Bearer ${token}` } }
        );
      }
  ...
  const handleDeleteIntern = async (internId) => {
    ...
    try {
      ...
      const deleteResponse = await axios.delete(`http://localhost:5000/api/interns/${internId}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      ...
      const pastResponse = await axios.get('http://localhost:5000/api/interns/past', {
        headers: { Authorization: `Bearer ${token}` }
      });
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      await axios.post(
        `http://localhost:5000/api/interns/${selectedIntern._id}/progress`,
